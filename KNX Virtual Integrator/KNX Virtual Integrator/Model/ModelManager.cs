    ...
    IApplicationFileManager applicationFileManager)
{
    ...
    public XDocument? LoadXmlDocument(string path)
    {
        try
        {
            return fileLoader.LoadXmlDocument(path);
        }
    ...
    {
        return fileFinder.FindFile(rootPath, fileNameToSearch);
    }
    ...
    public async Task SearchForZeroXmlAsync(string rootPath)
    {
        await fileFinder.FindZeroXml(rootPath);
    }
    ...
    // ------------------------------------------ LOGGER ------------------------------------------ //
    /// <summary>
    /// Writes a message to the console and logs it using the logger service.
    /// </summary>
    ...
    /// <summary>
    /// Writes a message followed by a newline to the console and logs it using the logger service.
    /// </summary>
    /// <param name="msg">The message to be written to the console and the log.</param>
    /// <exception cref="Exception">Thrown if an error occurs while writing to the console and logs.</exception>
    ...
    /// <summary>
    /// Writes a message to the log using the logger service.
    /// </summary>
    ...
    /// <summary>
    /// Writes a message followed by a newline to the log using the logger service.
    /// </summary>
    /// <param name="msg">The message to be written to the log.</param>
    /// <exception cref="Exception">Thrown if an error occurs while writing to the logs.</exception>
    ...
    {
        return projectFileManager.ExtractProjectFiles(knxprojSourceFilePath);
    }
    ...
    {
        return projectFileManager.ExtractGroupAddressFile(groupAddressesSourceFilePath);
    }
    ...
    {
        return projectFileManager.SelectAnotherFile();
    }
    ...
    {
        zipArchiveManager.CreateZipArchive(zipFilePath, paths);
    }
    ...
    {
        zipArchiveManager.AddDirectoryToArchive(archive, directoryPath, entryName);
    }
    ...
    {
        groupAddressManager.ProcessZeroXmlFile(groupAddressFile);
    }
    ...
    {
        groupAddressManager.ProcessStandardXmlFile(groupAddressFile);
    }
    ...
    public void AddToGroupedAddresses(Dictionary<string, List<XElement>> groupedAddresses, XElement ga,
        string commonName)
    {
        groupAddressManager.AddToGroupedAddresses(groupedAddresses, ga, commonName);
    }
    ...
    {
        groupAddressManager.SetNamespaceFromXml(zeroXmlFilePath);
    }
    ...
    {
        groupAddressManager.MergeSingleElementGroups(groupedAddresses);
    }
    ...
    {
        return groupAddressManager.AreNamesSimilar(name1, name2);
    }
    ...
    {
        return groupAddressManager.NormalizeName(name);
    }
    ...
    {
        return groupAddressManager.CalculateSimilarity(str1, str2);
    }
    ...
    {
        return groupAddressManager.LevenshteinDistance(str1, str2);
    }
    ...
    {
        return systemSettingsDetector.DetectWindowsTheme();
    }
    ...
    {
        return systemSettingsDetector.DetectWindowsLanguage();
    }
    ...
    {
        debugArchiveGenerator.CreateDebugArchive(includeOsInfo, includeHardwareInfo, includeImportedProjects);
    }
